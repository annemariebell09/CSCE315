...
/**
  ...
  /**
  *
  * @return List of titles that have a big user following, and
  * are rated higher than a four or five
  */
 public List<String> getCultClassics() {
   ...
   try{
     PreparedStatement sqlStatement = conn.prepareStatement(
    		 "SELECT originaltitle FROM titles " +
             "JOIN ratings ON titles.titleid = ratings.movieid " +
    		 "WHERE rating BETWEEN 4 AND 5 " +
             "GROUP BY originaltitle " +
    		 "ORDER BY MAX(numvotes); ");
 ...
 /**
  * @return List of top ten titles of the Hollywood pairs with the most chemistry
  */
 public List<String> getHollywoodPairs() {
   ...
   try {
     PreparedStatement sqlStatement = conn.prepareStatement(
       "SELECT originaltitle FROM titles " +
       "JOIN principals ON titles.titleid = principals.titleid" +
       "WHERE category = 'actor' OR category = 'actress' " +
       "GROUP BY originaltitle ORDER BY MAX(avgrun) " + 
       "LIMIT 10;");
